...
// Generate data from a rRUM model
//
...
arma::mat sim_rrum_main(const arma::mat &Q, 
...
// [[Rcpp::export]]
arma::mat sim_rrum(const arma::mat& Q,
                   const arma::mat& rstar,
                   const arma::vec& pistar,
                   const arma::mat& alpha) {
  
  if (Q.n_rows != rstar.n_rows || Q.n_cols != rstar.n_cols) {
    Rcpp::stop("Q and rstar must have the same dimensionaltiy");
  } else if (!arma::approx_equal( find( Q == 0 ), find( rstar == 0 ), "absdiff", 0)) {
  ...
  bool rstar_check= arma::all(arma::vectorise(arma::all(rstar >= 0 && rstar <= 1) == true));

  bool pistar_check = arma::all(pistar >= 0 && pistar <= 1);
  
  if(!(rstar_check &&
     pistar_check ) ) {
      Rcpp::stop("all entries of pistar and rstar must be bound between 0 and 1");
  }
